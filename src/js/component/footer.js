import React, { Component } from "react";

export const Footer = () => (
  <footer className="footer mt-auto py-3 text-center dark">
    <p>Copyright © 2023 Stars Music Entertainment. All Rights Reserved.</p>
    <p>
      We used a template from{" "}
      <a href="https://github.com/4GeeksAcademy">4Geeks Academy</a>{" "}
      <i className="fa fa-heart text-danger" />
    </p>
    <a className="link text-light text-decoration-none" href="/who">
      <img
        className="rounded"
        src="    data:image/png;base64,"
        style={{ width: "20px", height: "20px" }}
      />{" "}
      Creators
    </a>
    <br />{" "}
    <p>
      Happy coding! <i className="fa-solid fa-code" />
    </p>
  </footer>
);
